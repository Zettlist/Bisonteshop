...
/**
 * El codigo de Envia para el estado, o null si no se reconoce.
 *
 * Antes caia a 'CX' (CDMX) con cualquier nombre que no fuera exacto: "México"
 * desde Ajustes o "Nuevo Leon" sin acento se cotizaban como Ciudad de Mexico.
 * Ahora el nombre pasa por lib/estadosMx.js, y lo que no se reconoce no se
 * cotiza: un estado adivinado es un envio cobrado y mandado a otro lugar.
 */
export function getStateCode(estado) {
    const canonico = estadoCanonico(estado);
    return canonico ? STATE_CODES[canonico] || null : null;
}
...
export async function cotizarEnvio({ items, destination }) {
    ...
    if (!stateCode) return { carriers: [], estadoInvalido: true };

...
export function huellaDestino(dir) {
    ...
    // Sobre el nombre de la lista (lib/estadosMx.js): "Nuevo Leon" y "Nuevo
    // León" son el mismo destino y tienen que dar la misma huella. Para los
    // nombres de la lista el resultado es identico al de antes.
    const estado = String(estadoCanonico(dir?.estado) || dir?.estado || '').trim().toLowerCase().normalize('NFC');

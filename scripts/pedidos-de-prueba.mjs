...
const ESCENARIOS = [
    ...
    { titulo: 'Saldo + Mastercard — confirmado, por despachar', horas: 30, saldo: 200,
      carrito: [['normal', 1], ['adultos', 1]], dir: D.puebla, paqueteria: 'dhl', tarjeta: 'pm_card_mastercard',
      despues: async (p) => { await confirmar(p, 26); } },
    { titulo: 'Visa — empacado, esperando recolección', horas: 28,
      carrito: [['pesado', 1], ['adultos', 2]], dir: D.monterrey, paqueteria: 'estafeta', tarjeta: 'pm_card_visa',
      despues: async (p) => { await confirmar(p, 24); await despachar(p, 20); } },
    { titulo: 'Mastercard — empacado, esperando a DHL', horas: 26,
      carrito: [['barato', 2]], dir: D.cdmx, paqueteria: 'dhl', tarjeta: 'pm_card_mastercard',
      despues: async (p) => { await confirmar(p, 22); await despachar(p, 18); } },
    { titulo: 'Visa débito — empacado, esperando a DHL', horas: 25,
      carrito: [['caro', 1], ['normal', 2]], dir: D.guadalajara, paqueteria: 'dhl', tarjeta: 'pm_card_visa_debit',
      despues: async (p) => { await confirmar(p, 21); await despachar(p, 17); } },
    { titulo: 'Amex — empacado, esperando a FedEx', horas: 24,
      carrito: [['pesado', 1]], dir: D.merida, paqueteria: 'fedex', tarjeta: 'pm_card_amex',
      despues: async (p) => { await confirmar(p, 20); await despachar(p, 16); } },

...
export default function FigurasPage() {
    ...
    // Cargar figuras
    useEffect(() => {
        async function fetchFiguras() {
            try {
                const res = await fetch('/api/figuras');
                if (!res.ok) throw new Error('Error al cargar la base de datos');
                const data = await res.json();
                if (data.success) {
                    setFiguras(data.mangas); // Reusing 'mangas' array name from API for layout compatibility
                } else {
    ...
    const processedFiguras = useMemo(() => {
        let result = figuras.filter(m => m.is_adult !== 1 && m.is_adult !== true);
        ...
        // Tags: un artículo debe tener TODOS los tags seleccionados (ya sea en el array de tags o en su campo gender)
                        ...
                        {loading ? (
                            ...
                            <>
                                <p style={{ color: 'var(--muted)', marginBottom: '1.5rem', fontWeight: '600' }}>
                                    Mostrando <span style={{ color: 'var(--foreground)' }}>{processedFiguras.length}</span> de <span style={{ color: 'var(--foreground)' }}>{figuras.length}</span> artículos
                                </p>

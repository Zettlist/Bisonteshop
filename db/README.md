...
| `tests/` | 65 pruebas contra un MySQL 8 efímero. |
...
Se aplicaron durante la reconstrucción de jun 2026. Con datos en producción cada
una habría requerido su propia migración; sin datos costaron cero.

| # | Antes | Ahora |
|---|---|---|
| 1 | `sales` cargaba 8 columnas de envío, NULL en cada venta de mostrador | `bisonte_shipments` |
...
El fix 13 pide cambio de código además del esquema: quitar los `ensureTable()`
de `app/api/{notifications,addresses,credit,eventos/mundial,orders/capture,init-tables}/route.js`.
Los fixes 1 y 4 también — el checkout debe escribir `sale_items` y
`bisonte_shipments` en vez de `items_json` y las columnas de `sales`.
